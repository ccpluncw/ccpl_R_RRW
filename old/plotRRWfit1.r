...
plotRRWFit1 <- function (data, dataRtCol, dataPhitCol,  rtFitCol, pHitFitCol, correctCol, overlapCol, sCols = NULL, vcCols = NULL,  plotFilename, twoPlotsPerPage = TRUE) {
    ...
    if(!is.null(sCols) & !is.null(vcCols)) {
      sbVcConds <- unique(data[c(sCols, vcCols)])
      sbVcConds.n <- nrow(sbVcConds)

      for(svC in 1:sbVcConds.n) {
        df.tmp <- data[data[[sCols]]==sbVcConds[svC,sCols] & data[[vcCols]]==sbVcConds[svC,vcCols], ]
        pty <- 16 + svC - 1
        lty <- svC
        if(svC == 1) {
      ...
      if(!is.null(sCols)) {
        sbConds <- unique(data[[sCols]])

        for(sC in 1:length(sbConds)) {
          df.tmp <- data[data[[sCols]]==sbConds[sC],]
          pty <- 16 + sC - 1
          lty <- sC
          if(sC == 1) {
            plot(df.tmp[df.tmp[[correctCol]] == TRUE,overlapCol], df.tmp[df.tmp[[correctCol]] == TRUE,dataPhitCol], col=gray(0), pch=pty, ylim=c(0,1), xlab = xlab, ylab=NA)
        ...
        for(sC in 1:length(sbConds)) {
          df.tmp <- na.omit(data[data[[sCols]]==sbConds[sC],])
          pty <- 16 + sC - 1
          lty <- sC
          if(sC == 1) {
            plot(df.tmp[df.tmp[[correctCol]] == TRUE,overlapCol], df.tmp[df.tmp[[correctCol]] == TRUE,dataRtCol], col=gray(0), pch=pty, ylim=yMinMax, xlab = xlab, ylab=NA)
        ...
        if(!is.null(vcCols)) {
          vcConds <- unique(data[[vcCols]])

          for(vC in 1:length(vcConds)) {
            df.tmp <- data[data[[vcCols]]==vcConds[vC],]
            pty <- 16 + vC - 1
            lty <- vC
            if(vC == 1) {
              plot(df.tmp[df.tmp[[correctCol]] == TRUE,overlapCol], df.tmp[df.tmp[[correctCol]] == TRUE,dataPhitCol], col=gray(0), pch=pty, ylim=c(0,1), xlab = xlab, ylab=NA)
          ...
          for(vC in 1:length(vcConds)) {
            df.tmp <- na.omit(data[data[[vcCols]]==vcConds[vC],])
            pty <- 16 + vC - 1
            lty <- vC
            if(vC == 1) {
              plot(df.tmp[df.tmp[[correctCol]] == TRUE,overlapCol], df.tmp[df.tmp[[correctCol]] == TRUE,dataRtCol], col=gray(0), pch=pty, ylim=yMinMax, xlab = xlab, ylab=NA)
