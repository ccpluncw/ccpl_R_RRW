...
getRRWfit <- function(data, b, s=0, nSD=0, db=0, da=0.2, startBiasEffect = NULL, startBiasEffectCol = NULL, valueChangeEffect = NULL, valueChangeEffectCol = NULL, dataOverlapCol = "overlap", RwSamplesCol = "Q50", dataRtCol = "rt", dataPhitCol = "pHit", dataCorrectCol = "correct", loopsPerRWstep = 2000, plotFilename = NULL, sinkFilename = NULL) {
  ...
  #
  # yMinMax <- chutils::ch.getPlotAxisMinMax(df.fitted[[dataRtCol]])
  #
  # if(!is.null(startBiasEffectCol)) {
  #   sbConds <- unique(df.fitted[[startBiasEffectCol]])
  #
  #   for(sC in 1:length(sbConds)) {
  #     df.tmp <- df.fitted[df.fitted[[startBiasEffectCol]]==sbConds[sC],]
  #     pty <- 16 + sC - 1
  #     lty <- sC
  #     if(sC == 1) {
  #       plot(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataRtCol], col=gray(0), pch=pty, ylim=yMinMax)
  #     } else {
  #       points(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataRtCol], col=gray(0), pch=pty)
  #     }
  #     points(df.tmp[df.tmp$correct == FALSE,"overlap"], df.tmp[df.tmp$correct == FALSE,dataRtCol], col=gray(.6), pch=pty)
  #     with(df.tmp[df.tmp$correct == TRUE,], lines(overlap, rtFit, col=gray(0), lty=lty))
  #     with(df.tmp[df.tmp$correct == FALSE,], lines(overlap, rtFit, col=gray(.6), lty=lty))
  #   }
  #
  #   for(sC in 1:length(sbConds)) {
  #     df.tmp <- df.fitted[df.fitted[[startBiasEffectCol]]==sbConds[sC],]
  ...
  #     lty <- sC
  #     if(sC == 1) {
  #       plot(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataPhitCol], col=gray(0), pch=pty, ylim=c(0,1))
  #     } else {
  #       points(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataPhitCol], col=gray(0), pch=pty)
  #     }
  #     with(df.tmp[df.tmp$correct == TRUE,], lines(overlap, pCross, col=gray(0), lty=lty))
  #   }
  # } else {
  #
  #   if(!is.null(valueChangeEffectCol)) {
  #     vcConds <- unique(df.fitted[[valueChangeEffectCol]])
  #
  #     for(vC in 1:length(vcConds)) {
  #       df.tmp <- df.fitted[df.fitted[[valueChangeEffectCol]]==vcConds[vC],]
  #       pty <- 16 + vC - 1
  #       lty <- vC
  #       if(vC == 1) {
  #         plot(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataRtCol], col=gray(0), pch=pty, ylim=yMinMax)
  ...
  #     df.tmp <- df.fitted
  #
  #     plot(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataRtCol], col=gray(0), pch=16, ylim=yMinMax)
  #     points(df.tmp[df.tmp$correct == FALSE,"overlap"], df.tmp[df.tmp$correct == FALSE,dataRtCol], col=gray(.6), pch=16)
  #     with(df.fitted[df.fitted$correct == TRUE,], lines(overlap, rtFit, col=gray(0), lty='solid'))
  #     with(df.fitted[df.fitted$correct == FALSE,], lines(overlap, rtFit, col=gray(.6), lty='solid'))
  #     plot(df.tmp[df.tmp$correct == TRUE,"overlap"], df.tmp[df.tmp$correct == TRUE,dataPhitCol], col=gray(0), pch=16, ylim=c(0,1))
  #     with(df.fitted[df.fitted$correct == TRUE,], lines(overlap, pCross, col=gray(0), lty='solid'))
  #
  #   }
  # }
  #
